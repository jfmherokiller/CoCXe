...
public function WitchesSabbathThirdVisitYes():void {
	...
	if (player.hasCock() && player.catCocks() < player.cockTotal()) {
		...
		else outputText(" Pressure builds around " + (player.cockTotal() > 1 ? "their" : "its") + " base" + (player.cockTotal() > 1 ? "s" : "") + " as a newly formed sheath pulls " + (player.cockTotal() > 1 ? "them" : "it") + " into its fold. Only for " + (player.cockTotal() > 1 ? "them" : "it") + " to surge back as the shape of your " + (player.cockTotal() > 1 ? "many lenghts" : "entire length") + "");
