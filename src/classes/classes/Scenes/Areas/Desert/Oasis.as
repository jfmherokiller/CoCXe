...
private function oasisTalk():void {
	...
	outputText("<b>Do you stay or try to leave?</b>");
...
private function oasisTalkAccept():void {
	...
	if(player.isTaur()) outputText("your rear legs losing their balance and sending you crashing on your flank.  ");
	else outputText("legs falling open in the process.  ");
	outputText("Suddenly the silence is broken by a shrill screeching laugh, then a howl and the movement of the demons begins to accelerate. The deep bass laugh of the demon leader breaks over you like a crashing wave and the demons shriek with frenzied lust as they take you on the sand of the oasis.");
...
internal function oasisSexing():void {
	...
	if(player.cocks.length > 0) {
		...
		if(player.cocks.length > 1) {
			outputText("As you lie there under the needy demon your other ");
			if(player.cocks.length == 2) outputText(cockDescript(1) + " is");
	...
	if(player.vaginas.length > 0) {
		//Herms only!
		if(player.gender == 3) outputText("From your cock it is only a tiny trip downward between your legs before the demons discover something new to play with.  ");
		//Girlies only!
		else outputText("The demons quickly find your " + vaginaDescript(0) + " and tussle eagerly for position at your entrance, first with hands and then with a wide range of demonic dicks.  ");
		//gaping cunners!
	...
	outputText("Soon you feel hands and dicks grabbing and jabbing at your [butt], edging inwards bit by bit and jostling for position as a slippery cock positions itself at the opening of your " + assholeDescript() + ". It pauses for a second and then dives straight into your waiting asshole.  ");
	if(player.vaginas.length > 0) outputText("You feel the cocks in your ass and pussy rubbing into each other as your lower body becomes a temple of friction. The feeling of so much demon stuffing you as full as possible is almost unbearable, and you are inches away from blacking out as you come again.  ");
	outputText("The cock fucks you hard and fast for a dozen strokes and then you feel it begin to pump its cum into your stuffed ass until it can't come any more. It slips out of your [butt] and is quickly replaced by another which comes and then is itself replaced in what becomes a seemingly endless cycle.  ");
	player.buttChange(monster.cockArea(0), true);
	//More fucking!
	outputText("Around you you see the demons wrapped up in the frenzy of fucking that they've fallen into. No hole is unfilled and not a single cock isn't sunk into some being's flesh. The moans and screams are almost contagious and you feel them clouding your brain making you want more, harder.");
...
private function oasisBadEnd():void {
	...
	outputText("After passing out from your latest orgy with the desert tribe, you wake up to find yourself still naked and laying on your back. ");
		...
		if(player.isTaur()) outputText("Your four legs are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ");
	...
	//[If female/herm]
	if(player.gender >= 2) outputText("He reaches down to grab a hold of your hair and lifts you to your feet, causing you to yelp out in pain from the sharp pull.  ");
