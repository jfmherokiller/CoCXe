...
public function fapArenaGOOOO():void {
	...
	//[if dick size >= 8 inches: 1st time]
		...
		outputText("Everyone nods in agreement, hands and genitalia quivering in excitement.  ");
		//[if Corr >40]
		if(player.cor > 66) outputText("Sounds fun!");
		//[else]
		else outputText("By gods, what are you doing here?");
	...
	//[if dick size > 8 inches: after first time]
		...
		outputText("Everyone nods in agreement, hands and genitalia quivering in excitement.");
		//[if Corr >40]
		if(player.cor > 66) outputText("  You look forward to this new masturbating session.");
...
private function fapArenaPageII():void {
	...
	if(player.cor > 66) outputText("You eagerly seize the rod on your right side; squeezing it a bit in impatience.  A  little tingle alerts you to your left neighbor grabbing " + oMultiCockDesc() + " with an expert hand.  Everyone around you quickly grabs hold of the nearest cock on their right side.  Some of them are trembling because of the induced stress and arousal.\n\n");
		...
		outputText("You hesitantly pick the rod on your right side with an awkward hand.  You nearly let it go because you're so nervous, and its owner smiles: you're obviously not very experienced with dicks.  ");
	...
	if(player.hasVagina()) outputText("other ");
...
private function fapResults(place:Number = 3):void {
	...
	if(place == 3) {
		...
		outputText("But it doesn't matter now.  None of it matters, as your own " + cockDescript(x) + " is about to release a flood of its own.  After a whole orgasm self-denying session, you feel like your bones are melting and your body is ascending to heaven as you reach your peak and your " + cockDescript(x) + " starts shooting an ");
		if(player.cumQ() > 1000) outputText("unholy");
		else if(player.cumQ() >= 500) outputText("enormous");
		else outputText("appreciable");
		//(depending on cum production)
		outputText(" rope of the creamiest, whitest liquid you've ever produced.");
		if(player.cockTotal() > 1) outputText("  Several more gouts follow suit from the unstimulated dickflesh dangling from your groin.");
		...
		if(player.hasVagina()) {
		outputText("  Your thighs tense a few seconds before your " + vaginaDescript(0) + " squirts too, forming a ");
			if(player.wetness() >= 4) outputText("large puddle");
			else if(player.wetness() >= 2) outputText("puddle");
	...
	//[else if the player doesn't lose, but doesn't win either - he cums neither first nor last]
		...
		if(player.cumQ() >= 1000) outputText("soak");
		else if(player.cumQ() >= 500) outputText("pollute");
		else outputText("wet");
		outputText(" the floor with your sexual juices and yell in a whorish voice, \"<i>NUMBER " + (num+4) + " CUMMING!</i>\"; you're almost immediately followed by the scoring announcement from your left neighbor.  With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + cockDescript(x) + " at her, trying to soak her too.  She hardly dodges it and some lands on her arms, but she doesn't seem to notice.");
		if(player.hasVagina()) {
			outputText("  Your thighs tense a few seconds before your " + vaginaDescript(0) + " squirts too, forming a ");
			if(player.wetness() >= 4) outputText("large puddle");
			else if(player.wetness() >= 2) outputText("puddle");
	...
	//[else, the player wins]
		...
		outputText(" the floor with your sexual juices and yell in a whorish voice: \"<i>NUMBER " + num+ " CUMMING!</i>\"  With a half-closed eye you notice the referee nodding in acknowledgement and you point your " + cockDescript(x) + " at her, trying to soak her too.  She hardly dodges it and some lands on her arms, but she doesn't seem to notice.");
				if(player.hasVagina()) {
			outputText("  Your thighs tense a few seconds before your " + vaginaDescript(0) + " squirts too, forming a ");
			if(player.wetness() >= 4) outputText("large puddle");
			else if(player.wetness() >= 2) outputText("puddle");
	...
	//[if corr < 50]
	if(player.cor < 50) outputText("You find yourself yelling with them, astounded by everyone's enthusiasm and eagerness to be fucked in the ass, even after this rough masturbating session.  You wish you weren't so blindly following others, but they are so hypnotizing, their hard glistening cocks pointing at the air as they keep shouting over and over again, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\"  ");
	//[else, corr >50]
	else outputText("Your body tenses and " + sMultiCockDesc() + " hardens at the perspective of a collective anal orgy.  You yell in lust like a minotaur in rut, \"<i>BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!  BUTT-FUCK!</i>\" along with the others.  ");
	//end of global condition about PC's corr, display the following
	...
	//[if you won]
	if(place <= 1) outputText("Since you're the winner, you walk to the tail of the butt-fuck train, enjoying your prize.  ");
	//[if you didn't win, didn't lose]
	else if(place <= 2) outputText("You place yourself between two horny bodies.  The lust and confusion have obscured your senses and all you see is a rock-hard dick and a gaping anus, both waiting for you to play with.  ");
	//[if you lost]
	else {
		outputText("You meekly place yourself at the end of the butt-fuck wagon, knowing you'll have to endure the thrusts of everyone else.  In your shame and lust, all you can see is a member of enormous size, trembling in arousal, waiting to lodge between your ass-cheeks.  With ");
		//([if corr < 50]
		if(player.cor < 50) outputText("a whorish smile");
		else outputText("apprehension");
		outputText(", you place yourself in front of the creature with the large cock.  It grinds against your back in impatience.  ");
	}
	...
	if(place <= 1) {
		...
		if((player.cockTotal() > 2) || (!tent && player.cockTotal() > 1)) {
			...
			else outputText("good");
			outputText(" spray of ");
			if(player.cockTotal() > 3 || (!tent && player.cockTotal() > 2)) outputText("their ");
			else outputText("its ");
			outputText("own, unhindered by the tight colon holding your first " + cockDescript(x) + ".");
		}
		//[if player has vagina]
		if(player.hasVagina()) {
			outputText("  Your feminine part squirts a ");
			if(player.wetness() >= 5) outputText("titanic");
			else if(player.wetness() >= 3) outputText("massive");
			else outputText("nice");
	...
	//[else if you didn't win]
		...
		outputText("You almost instantly feel firm arms groping your [butt] and the tip of an erect dick knocking at the door of your colon.  It slowly massages your rectum, and you can't help but giggle a bit as you feel a tingle of pleasure passing through your body.\n\n");
	...
	//[else, if you lost]
		...
		if(player.lib > 50) outputText("  Maybe losing wasn't that bad? You feel like you could lose a few more times...");
		outputText("\n\n");
	}
	...
	//[if you didn't win]
		...
		outputText("You awkwardly remove the dick from your ass and start dressing yourself.  Before you go, the cute little referee waves at you, her coy smile and nude body somehow awakening " + sMultiCockDesc() + " again.  Ye Gods, it never ends.  \"<i>I hope you enjoyed this session. Better luck next time...</i>\"");
...
private function tentacleFapSpecial(place:Number):void {
	...
	if(player.cocks[x].cockLength >= 40 && player.cocks[x].cockType == CockTypesEnum.TENTACLE) {
		...
		if(place <= 2) {
			outputText("Needless to say, this interference creates a general disruption in the whole butt-fuck wave.  The whole train of peckers and puckers starts moving irregularly, grinding against your " + cockDescript(x) + " at the most unexpected moments, and the pleasure you're getting from your extra tentacle cock doesn't help.  Everyone's butt and dick is affected to some extent; everyone's pleasure is increased because of your audacious initiative, and loud, reluctant moans are heard.");
