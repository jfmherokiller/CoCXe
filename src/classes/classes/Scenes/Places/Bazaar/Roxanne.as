...
public class Roxanne extends BazaarAbstractContent implements TimeAwareInterface {
    ...
    private var autoLose:Boolean = false; // -Is PC losing the Roxanne's drinking contest intentionally?
    ...
    public function timeChange():Boolean {
        ...
        flags[kFLAGS.ROXANNE_DICK_SIZE]++;
        //Reset if she finds someone to take it (random at high values)
        if (flags[kFLAGS.ROXANNE_DICK_SIZE] >= 300 && model.time.hours == 1 && rand(5) == 0) flags[kFLAGS.ROXANNE_DICK_SIZE] = 1;
        if (model.time.hours > 23 && flags[kFLAGS.ROXANNE_BONUS_SCORE] > 0) flags[kFLAGS.ROXANNE_BONUS_SCORE]--; //Reduce drinking contest bonus
        return false;
    }
    ...
    public function RoxanneChooseApproachOrRepeat():void {
        ...
        if (flags[kFLAGS.ROXANNE_MET] == 0) {
            ...
            if (player.cor < 33) outputText(", but would a champion really do something like that?");
            else outputText(".");
            outputText("\n\n");
            ...
            outputText("Do you engage her in a drinking competition?");
        }
        //[Approach – met but not drank yet]
        else if (flags[kFLAGS.ROXANNE_TIMES_WON] + flags[kFLAGS.ROXANNE_TIMES_LOST] <= 0) {
            outputText("\"<i>I see you've come back again [name].   Did you come back for the free drinks, or... ?</i>\" Roxanne trails off, wiggling her tongue at you.  She pats the huge, barely concealed bulge and asks, \"<i>Or did you want to lose?  Remember, if I win I get to plug your ass with this beast.  If you win, I'll give you oral service that only a lizard can.  Now come on, let's drink until we forget this wretched place.</i>\"\n\n");
            outputText("Do you engage Captain Poisontail in a drinking contest?");
        }
        //[Approach – lost last contest and she's huge]
        else if (flags[kFLAGS.ROXANNE_LAST_RESULT] == 2 && flags[kFLAGS.ROXANNE_DICK_SIZE] >= 200) {
            outputText("\"<i>Oooh, praise the sea god Ulrun, you've returned.  [name], it's been a while, and I've gotten sooooo big,</i>\" cheers an ecstatic Roxanne.  To emphasize her point she shows the seam-splitting bulge in her pants, rubbing it from the base all the way down to the tip, which rests next to her knee.  She's so big, and a damp spot appears at her pant-leg's knee while the huge cock-sausage visibly inflates.  Captain Poisontail asks, \"<i>So, can I count on you to get sauced and help me take care of this beast again, or do you actually think you have a chance of winning now?</i>\"\n\n");
        ...
        autoLose = false; //In case if lizans codex entry didn't unlock.
        camp.codex.unlockEntry(kFLAGS.CODEX_ENTRY_LIZANS);
    ...
    private function roxanneDrinkingContest():void {
        ...
        clearOutput();
        outputText("Roxanne ");
        if (flags[kFLAGS.ROXANNE_DICK_SIZE] >= 200) outputText("stumbles over her huge manhood, working towards");
        ...
        if (flags[kFLAGS.ROXANNE_TIMES_WON] + flags[kFLAGS.ROXANNE_TIMES_LOST] <= 0 || flags[kFLAGS.ROXANNE_LAST_RESULT] == 2) {
            ...
            if (flags[kFLAGS.ROXANNE_LAST_RESULT] == 2) outputText(" after your last humiliation.");
        ...
        outputText("  You ");
        if (flags[kFLAGS.ROXANNE_TIMES_WON] + flags[kFLAGS.ROXANNE_TIMES_LOST] < 3) outputText("hesitantly sniff at the brew, taking in its dark color and heady, hoppy aroma before");
        ...
        if (!autoLose) score = (player.tallness * ((player.thickness + 100) / 200) * (player.tou / 100)) + flags[kFLAGS.ROXANNE_BONUS_SCORE];
        //If score is less than 30-50 (Strahza is inconsistant!)
        //[Lose!]
        if (score < (45 + rand(20))) {
            //Increment loss count!
            flags[kFLAGS.ROXANNE_TIMES_LOST]++;
            //Set who won contast last
            flags[kFLAGS.ROXANNE_LAST_RESULT] = 2;
            //Gain big bonus
            flags[kFLAGS.ROXANNE_BONUS_SCORE] += 10;
            ...
            if (!autoLose) outputText("Nooooo! You're cut off! That means Roxanne won...");
			...
			var canFatLose:Boolean = player.butt.type > 12 && player.tone <= 50 && flags[kFLAGS.ROXANNE_TIMES_LOST] > 1;
            if (canFatLose && rand(2) == 0) doNext(bigBootyRoxanneContestLoss);
            //TO huge or regular anal
            else {
				if (!canFatLose) sceneHunter.print("Check failed: fat ass, low muscle tone, at least one loss.");
				if (flags[kFLAGS.ROXANNE_DICK_SIZE] >= 200) doNext(roxanneFucksYourAssOHGODITSHUGE);
				else {
					sceneHunter.print("Check failed: HUGE Roxanne's dick.");
					doNext(roxanneReamsYouNormal);
				}
			}
            //Reset roxanne's cock
            flags[kFLAGS.ROXANNE_DICK_SIZE] = 1;
        }
        //[WIN]
        else {
            //Increment win count!
            flags[kFLAGS.ROXANNE_TIMES_WON]++;
            //Set who won contest last
            flags[kFLAGS.ROXANNE_LAST_RESULT] = 1;
            //Gain small bonus
            flags[kFLAGS.ROXANNE_BONUS_SCORE] += 4;
            ...
            menu();
            addButtonIfTrue(0, "Cunnilingus", roxanneCunnilingus, "Req. a vagina.", player.hasVagina());
            addButtonIfTrue(1, "Fellatio", roxanneGivesABlowjob, "Req. a cock.", player.hasCock());
    ...
    private function roxanneCunnilingus():void {
        ...
        outputText(" the ");
        if (player.clitLength < 1) outputText("little pleasure buzzer");
        else if (player.clitLength < 3) outputText("swollen love-button");
        else outputText("cock-like lady-part");
        outputText(", slobbering over it with a lewd tongue-kiss. Her frothy spit completely soaks it ");
        ...
        if (player.hasFuckableNipples()) {
            outputText("Roxanne pulls her hands off your ass and reaches up, swaying unsteadily until she catches your " + nippleDescript(0) + "s.  A second later she slips her digits inside the welcoming tit-pussies, finger-fucking your breasts as ");
            if (player.biggestLactation() < 1.5) outputText("expertly as the one below");
            else outputText("they begin to squirt milk around her");
            outputText(".  ");
        }
        outputText("Unable to dam the pressure up inside you any longer, you cum with brain-breaking, mouth-babbling intensity.  The lizan doesn't relent in the slightest as you begin to spasm");
        if (player.wetness() >= 5) outputText(" and squirt");
    ...
    private function roxanneRimjob():void {
        ...
        if (player.hasCock()) {
            ...
            if (player.cumQ() < 500) {
            } else if (player.cumQ() < 1000) outputText("  It forms a nicely-sized pool before the ground wicks it up.");
            else if (player.cumQ() < 1500) outputText("  It forms a large pool before the ground soaks it up.");
            else outputText("  It forms a massive pool before the ground soaks it up.");
        } else if (player.hasVagina()) {
            outputText("  You quiver and cum on her fingers, ");
            if (player.wetness() >= 4) outputText("soaking them with your gushing orgasms ");
            else outputText("sliming them with copious girl-cum ");
            outputText("while she teases your " + clitDescript() + " with those terribly-slow touches.");
        }
        outputText("\n\n");

        outputText("Completely finished, you can do naught but tremble while Roxanne disentangles her tongue from your rectum, popping from your backside with a noisy slurp.  She immediately falls over and giggles, completely drunk, but she somehow staggers back up to give your ass a smack.  You follow her example and pull yourself to your feet, tired from the anal orgasm but feeling quite relaxed.  Once you've readied yourself, you realize that Roxanne's crew are arranged in a semicircle around you.  You note a number of demons are on the prowl beyond that protective ring, though they seem to be dispersing now that the act has finished.  You make your way back to check on camp once it looks safe.");
        //(-100 lust, -1 int
    ...
    private function roxanneReamsYouNormal():void {
        ...
        player.buttChange(Math.floor(30 + (flags[kFLAGS.ROXANNE_DICK_SIZE] / 4)), true, true, false);
        outputText("\n\n");
        ...
        //(JUST CUNTS:
            ...
            else if (player.wetness() <= 3) outputText("soaking your ready vulva.");
            else if (player.wetness() <= 4) outputText("which drip from your ready vulva.");
        ...
        outputText("Roxanne tugs back, but even with the pressure of her reproductive leavings inside you, you keep her pinned inside your depths, squeezing and writhing on her shaft.  She tugs harder and harder, muscles going taut under her shiny purple scales until, with a violent 'POP', she slides free.  You climax at that moment, getting off while a river of cum pours from your gaping asshole.  Crying and moaning, you tremble while your beer-stained lips drool in bliss.");
        if (player.hasCock()) {
            outputText("  ");
            if (player.cumQ() >= 1500) {
                outputText("A torrential outpouring of spunk pours from " + sMultiCockDesc() + ", leaking in a steady, submissive flow to grow a ");
                if (player.cumQ() <= 2500) outputText("puddle");
                else outputText("lake");
                outputText("below the table.");
            } else outputText("An outpouring of spunk drools from " + sMultiCockDesc() + ", spurting submissively under the table.");
        }
    ...
    private function roxanneFucksYourAssOHGODITSHUGE():void {
        ...
        outputText("to examine your " + assholeDescript() + ", licking the ring before she prods it with a gentle fingertip.  The excited pirate exclaims, \"<i>");
        ...
        outputText("-slicked ass to accept ever-greater quantities of cock.  Sharp fingernails drag over your " + hipDescript());
        if (player.skinType == Skin.PLAIN) outputText(", leaving red lines in your unblemished skin");
    ...
    private function bigBootyRoxanneContestLoss():void {
        ...
        outputText("\n\nYou're openly moaning and gasping, your face pressed into the mattress by the hard-fucking lizan.  She pounds you faster and faster, breathily exulting in the pleasure each time she bottoms out against your gigantic backside.  You can't help but clench and squeeze either - your body is being battered relentlessly.  Your sphincter convulses around the thick cock, and when you can't take anymore, you cum, babbling drunkenly and submissively.");
    ...
    private function applyHangover():void {
        ...
        player.statStore.addBuffObject({'str': -5, 'spe': -10, 'int': -15}, 'Hangover', {
            text: 'Hangover',
