...
public function edrynSprite():void {
...
public function edrynBarTalk():void {
	...
	if(flags[kFLAGS.EDRYN_NEEDS_TO_TALK_ABOUT_KID] == 1) {
		...
		if(flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] == 1) {
			...
			outputText("I can't believe we've had " + num2Text(flags[kFLAGS.EDRYN_NUMBER_OF_KIDS]) + " kids together!  You better come see me later on - ");
			if(flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] < 3) outputText("I miss the way you fill me");
			else outputText("I want you to make me pregnant again");
		}
		outputText(".</i>\"\n\n");

		outputText("You stay with her and chat, learning more about your newborn child and otherwise having a pleasant time with your quadruped lover.");
		cheatTime(1);
	...
	//Mid-pregnancy talk
		...
		outputText("Edryn smiles pleasantly as you approach, ");
	...
	//Post kids talk
		...
		if(player.cockArea(x) < 300 && player.cockArea(x) > 24) {
			outputText("She winks at you as she gets up and trots off, giving her butt a sensual sway to draw your eyes.  Her potent scent hangs in the air, and your body reacts immediately and intensely, flooding you with arousal.  ");
			outputText("You look down at your [cocks] and curse, irritated at how easily she can affect you.  There's no way you'll be turning her down this time.  You get up and follow her back to her room, intent on taking care of the need between your legs.\n\n");
			//Sex
			doNext(pregdrynOffer);
			return;
		}
		//(Too small)
			...
			outputText("She looks down, eyes fixating on your crotch for a moment.  Edryn asks, \"<i>When did you get so small?  We can't fuck like this!  ");
	...
	outputText("Edryn smiles pleasantly as you approach, ");
	if(player.isTaur()) outputText("offering you a spot at the table across from her.  You realize your companion isn't on any kind of seat at all, and is instead 'sitting' on her lower half.  You do the same, settling down across from her, thankful this bar was made to accommodate centaurs.");
	else outputText("gesturing to a nearby stool.  You grab the seat and realize that your centaur companion isn't sitting still at all, but is instead 'sitting' on her lower half, which currently lies on the floor.");
	...
	if(player.isTaur() && player.cockTotal() > 0) {
		...
		outputText(", now rock-hard.  You breathe deeply, your mind subsumed in a sea of feral instincts.  The hard floor squeezes your [cock] painfully underneath you, forcing you to rise up.  You can feel more than a few curious gazes sliding along your now fully exposed maleness, and it twitches as if it were happy for the attention.\n\n");
		...
		//Non horsedick
			...
			if(player.cor > 50 || player.statusEffectv1(StatusEffects.Edryn) > 0 || flags[kFLAGS.LOW_STANDARDS_FOR_ALL]) {
				outputText("  Unbidden, your mind wonders what her juicy horse-snatch would feel like, and your [cock] responds immediately, thickening with readiness.  You squirm uncomfortably from how constricting your [armor] feels.\n\n");
...
private function getCost():int {
	if (player.statusEffectv1(StatusEffects.Edryn) >= 4) return 0;
...
private function edrynOffer(x:Number):void {
	...
	if(player.isTaur() && player.cockTotal() > 0) {
		...
		if(player.cockArea(x) < 24) {
			...
			outputText("You're a bit disappointed with the outcome. It doesn't look like you'll be getting any centaur tail tonight.");
			cheatTime(1);
			doNext(telAdre.barTelAdre);
			return;
		}
		...
		if(player.horseCocks() > 0) {
			...
			if(player.cockArea(x) >= 24) {
				...
				outputText("She doesn't seem irritated, instead she seems quite... turned on.  Edryn explains, \"<i>" + player.mf("Mr.","Miss") + " thick and juicy, I'm more than muscle for hire.  I do a little 'wetwork' on the side, and I've yet to meet a horse-cock that doesn't jump at my scent.  Lucky for you, I think you're cute, and I'm feeling randy.  So, how about ");
				if(cost > 0) outputText(num2Text(cost) + " gems, ");
				else outputText("an hour of your time, ");
		...
		//Normal Wingdangdoodle
			...
			//(cont Normal)
				...
				outputText("Edryn keeps talking, \"<i>I do ALL kinds of mercenary work, even the sloppy wet kind, and lucky for you, I'm more than wet enough to let you satisfy your [cock] in me.  ");
				if(cost > 0) outputText(Num2Text(cost) + " gems ");
				else outputText("An hour of your time ");
				outputText("gets you the ride of a lifetime.  So what do you say [name]?  Do you want to be my sex-toy?</i>\"\n\n");
				outputText("She stands up and turns, exposing her exotic hind-end to you.  A drop of fluid slips from her folds and splatters under the table, reminding you just how wet this trick would be.  Do you really want to do her?");
...
public function edrynSexSelecter():void {
	...
	if(cost > player.gems) {
		clearOutput();
		outputText("You realize you can't afford to stay with the sexy centaur, and leave full of disappointment and arousal.");
		cheatTime(1);
		doNext(telAdre.barTelAdre);
		return;
	}
...
private function fuckEdrynTaur():void {
	...
	if(player.cor < 75) outputText("You recoil for a second, scolding yourself for the strange thoughts, but you don't slow down the fucking in the slightest.");
	else outputText("You smile, realizing you hadn't meant to think of her as a mare and you as her stud, but that's exactly what you are right now.  Your thrusts speed up as you renew your determination to fill this cunt full of fillies.");
	...
	outputText("The rippling and squeezing of Edryn's cunt throws you past the point of no return.  Your eyes roll back as you unload into her musky tunnel, nearly losing your grip as your body gives up wave after wave of seed.  The contractions don't let up");
	if(player.cumQ() < 100) outputText(" even after you've finished.");
	else {
		outputText(", squeezing more and more cum into her horse-sized womb.  ");
		if(player.cumQ() > 250) outputText("Her body gurgles noisily from the massive fluid injection it's just taken, and her belly expands noticeably.  ");
		if(player.cumQ() > 500) outputText("It keeps going as your body spends the last of its massive reserves, bloating her to the point that she'll have trouble walking.  ");
		outputText("Her cunt keeps milking you even after.");
	}
...
private function fuckEdrynNonTaur():void {
	...
	var x:Number = player.cockThatFits(300);
	if(x < 0) x = 0;
	outputText("She wiggles her more than ample backside towards you, squelching wetly against your ");
	if(player.tallness < 48) outputText("face");
	...
	outputText(", pressing your [face] into her slick folds and tasting her flavor.  Your nose easily slips into her gash as your tongue explores it, testing and tasting thick, liquid centaur lust.  It's tangy and sweet, surprisingly unlike the scent it gives off.  You easily find her large clit and suckle on it, listening to the wet squelches and whinnies of pleasure.\n\n");
	...
	outputText("Your eyes roll back as you unload into Edryn's musky tunnel, giving up on breathing as your body gives up wave after wave of seed.  The contractions don't let up, ");
	if(player.cumQ() < 100) outputText("even after you've finished.\n\n");
	else {
		outputText("squeezing more and more cum into her horse-sized womb.  ");
		if(player.cumQ() >= 250) outputText("Her body gurgles noisily from the massive fluid injection it's just taken, and her belly expands noticeably. ");
	...
	outputText("As the edges of your vision start blacking out, Edryn ");
	if(player.hasKnot(x)) outputText("tries to rise, lifting up enough for you to get a few good breaths in, but your knot keeps her locked to you, and the two of you are tied together for another minute or two while it deflates, emptying the very last dregs of your spunk into her.  You pop free, ");
	else outputText("slowly rises up off of you, ");
	...
	else if(player.statusEffectv1(StatusEffects.Edryn) <= 3) outputText("She sighs happily and looks down at you, \"<i>Ooh [name], you're the best.  We should do this again soon.  Oh goddess, I can feel my juices starting just thinking about it.</i>\"\n\n");
...
private function leaveHelInZeBitch():void {
	...
	if(flags[kFLAGS.HEL_EDRYN_OFFER] == 1) {
		...
		outputText("You spend the rest of the hour quietly chatting with Helia before giving her a friendly kiss goodbye and stepping away.");
	}
	doNext(telAdre.barTelAdre);
}
...
public function helEdrynThreeSomeStartYerEngines():void {
	...
	if(flags[kFLAGS.EDRYN_TIMES_HEL_THREESOMED] == 0) {
		...
		outputText("\"<i>Hey, [name],</i>\" Edryn says with a half-smile. \"<i>Who's your friend?</i>\"\n\n");
	...
	//Repeat -- Threesome
		...
		outputText("Grinning, the three of you make your way up to Edryn's private chambers.");
	}
...
private function beAnAssholeToPregdryn():void {
	...
	outputText("<b>(Edryn will no longer speak with you.)</b>");
...
private function pregdrynOffer(cs:Boolean = true):void {
	...
	if (pregnancy.incubation < 250) {
		...
		if(player.cor + player.lib < 100 || player.cockTotal() < 1) outputText("you struggle not to reach into your [armor] to touch yourself.");
		else {
			outputText("you can't stop yourself from grabbing ");
			if(player.cockTotal() == 1) outputText("your ");
			else outputText("a ");
			outputText(cockDescript(0) + " and stroking it under the table.");
		}
		outputText("\n\n");
	...
	if(player.cockArea(x) >= 24 && player.cockArea(x) < 300) {
		...
		outputText("The centaur doesn't wait for a reply, and takes off at a trot towards her quarters.  A few drips have splattered across the floor, leaving a trail for you to follow.  You get up, dazed with arousal and leaking pre-cum, and stagger through the bar to her room, practically lust-drunk.");
	...
	//(PC TOO BIG)
		...
		outputText("Edryn brushes her hand over ALL of your [cock] then jerks it back, startled.  She sighs, \"<i>Dear, that thing is a BEAST.  I mean, there's no doubt I'd love to get it inside me, but I promise it won't fit me.</i>\"\n\n");
	...
	//(PC TOO SMALL)
		...
		outputText("The centaur doesn't wait for a reply, and takes off at a trot towards her quarters.  A few drips have splattered across the floor, leaving a trail for you to follow.  You get up, dazed with arousal and leaking pre-cum, and stagger through the bar to her room, practically drunk on lust.");
		//Go to 'too big or too small eat out'
		doNext(jizzFromEatingPregdrynOut);
		dynStats("lus", 50, "scale", false);
	}
...
public function fuckPregEdryn():void {
	...
	if(!player.isTaur()) {
		...
		outputText("Edryn lurches forwards as soon as the door closes behind you, slamming her powerful frame into you with enough force to propel you several feet back onto a large pile of pillows in the corner.  The feeling of her massive, milk-drooling teats ");
		if(player.tallness < 60) outputText("bouncing against the top of your head");
		else if(player.tallness < 84) outputText("battering your face");
		else outputText("crushed against your chest");
		outputText(" was totally worth the bruise-inducing impact.  Edryn pivots about, her hooves clattering noisily against the room's floorboards until she's presenting her hind end to you.  Her tail lifts of its own accord and displays the swollen, black lips of her sex.  A potent glaze of centaur-fluid drips from the gash in a steady trickle, splattering over your [feet] as your equine lover closes in.\n\n");

		outputText("You tear off your [armor] in a flash, fully exposing your ");
		if(player.cockTotal() > 1) outputText("chosen ");
		...
		outputText("Edryn sinks down, her whole body shaking and shuddering while milk fountains from her swollen breasts.  Her hands reach up to grab her nipples and tug on the milk-spouts.  They look longer now than before her pregnancy.  Her bloated milk-spouts are at least an inch or two long!  ");

		if(player.cumQ() < 100) {
			outputText("Your " + cockDescript(x) + " dumps the last of its seed into her hungry cunt and softens slightly.  Her warm cunt's constant milking motions continue, still pleasurable, but they prevent you from going soft until the centaur's orgasm has concluded.");
		}
		else if(player.cumQ() < 500) {
			outputText("Your " + cockDescript(x) + " spurts thick flows of seed into her hungry cunt, flooding her snatch with an abnormally large load of semen.  Her warm cunt's constant milking motions continue, forcing more and more of your spunk inside her until her own orgasm has concluded.");
		}
		else {
			outputText("Your " + cockDescript(x) + " erupts, pouring a thick wave of seed into the centaur's hungry cunt.  The constant milking motions don't let up while Edryn's orgasm continues, helping you flood her with even more spooge.  Even after your pregnant lover's orgasm has concluded, your " + cockDescript(x) + " keeps dumping more cum into her until it's soaking and dripping from her huge, furry ass.");
		}
	...
	//TAURZILLA
		...
		outputText(" to " + player.cockHead(x) + ", culminating in a tight squeeze at the tip. Then it releases and starts all over again.  Instinctively, you whinny and explode inside the warm, soaking wet tunnel.  The french-kiss turns into a feverish slobber-fest while the two of you mate, lost to orgasm.");
		if(player.cumQ() < 300) {}
		else if(player.cumQ() < 1000) outputText("Your prodigious jism output soaks her sloppy tunnel and drips from the entrance with the centauress' girlcum.");
		else {
			outputText("Your body's cum-production easily fills her channel to capacity.  Each successive spurt blasts a wave of fluid out from her overstuffed cunt, soaking the pillows, Edryn's ass, and your crotch.");
			if(player.cumQ() >= 4000) outputText("  By the time you calm down you've soaked all the pillows and spooge is puddling underneath.");
		}
		outputText("\n\n");

		outputText("Edryn breaks the kiss with a strand of spit hanging in the middle.  She giggles euphorically, \"<i>Wow.  Ummm, wow!  That was nice!  Now get off me you ");
		if(player.tallness > 74) outputText("big ");
...
private function jizzFromEatingPregdrynOut():void {
	...
	outputText("The smell is potent, sexual, and thoroughly inhuman.  You take a step closer and continue taking little sniffs, barely noticing as your ");
	if(player.cockArea(x) < 24) outputText("small dick begins to twitch and leak.");
	else outputText("mammoth cock drags the ground, leaking pre-cum.");
	...
	else if(player.cumQ() < 500) outputText("leaks pre-cum in a steady stream, quickly soaking a few pillows.");
	else outputText("pours out enough pre-cum to soak a pillow, and in no time your overenthusiastic dick is doing its damnedest to dribble over everything.");
	...
	if(player.isTaur()) {
		outputText("  Your legs twitch weekly on the floor, forgetten about as you focus entirely on your hands, mouth, and pulsating " + Appearance.cockNoun(CockTypesEnum.HUMAN));
	...
	outputText("Startled from your sexual fog, you jerk back and gasp.  The smell – it's like sex distilled into orgasm and fired straight into your brain.  Your " + hipDescript() + " shake uncontrollably, spasming wildly as your scent-addled mind sets off a full-body orgasm.  ");
	if(player.hasVagina()) {
		outputText("Your " + vaginaDescript(0) + " contracts and spasms with the rest of you, ");
	...
	if(player.cumQ() < 1000) {
		...
		if(player.cumQ() < 3000) outputText("out the last of your liquid pleasure.");
		else outputText("ever greater amounts of liquid peasure.  Jism drips through the floorboards as you create a puddle a few inches deep.");
	}
	outputText("\n\n");
	...
	outputText("\"<i>Turn-about is fair play!</i>\" she exclaims. You leave, unable to dispute the logic.");
...
private function edrynFucktroduction():void {
	...
	var cost:int = getCost();
	if(!player.isTaur()) {
		if(cost > 0) outputText("Coins exchange hands and her arm finds its way back inside your [armor], pulling you towards a back room.  A round of chuckles chases you through the barroom until they're silenced by the heavy thud of a door closing behind you.  You're pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ");
		else if(flags[kFLAGS.EDRYN_NUMBER_OF_KIDS] == 0) outputText("Her arm finds its way back inside your [armor], pulling you towards a back room.  A round of chuckles chases you through the barroom until they're silenced by the heavy thud of a door closing behind you.  You're pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ");
		else outputText("A round of chuckles chases you through the barroom until they're silenced by the heavy thud of a door closing behind you.  You're pushed against the wall hard enough to make your teeth click.  The stars gradually clear from your view, revealing a massive gash, drooling with clear feminine slime.  The scent assails you, ");
		if(player.cocks[0].cockType == CockTypesEnum.HORSE) outputText("making your " + cockDescript(x) + " twitch and ache.  It floods your body with animalistic desires, overpowering any sense of propriety you might have once had.");
		else outputText("overpowering you with strange lust.  As turned on as you are, it's difficult not to enjoy this.");
	}
	//TAUR
	else {
		if(cost > 0) outputText("Coins exchange hands and she grabs you, pulling you back towards a darkened doorway in the rear of the bar.  A round of chuckles chases you through the barroom until they're silenced by the heavy thud of a door closing behind you.  Edryn turns away from you, exposing her massive gash, drooling with clear feminine slime.  The scent assails you, ");
	...
	//[Fuck] [Eat Her Out]
	menu();
	addButton(0,"Eat Her Out",eatEdrynPussyLikeABawss);
	addButton(1,"TaurFuck",fuckEdrynTaur).disableIf(!player.isTaur(), "Req. to be a centaur too!");
	addButton(2,"Fuck",fuckEdrynNonTaur).disableIf(player.isTaur(), "Req. non-tauric lower body.");
}
...
public function eatEdrynPussyLikeABawss():void {
	...
	outputText("Edryn starts to say something, but you strip out of your [armor] before she gets more than a half-dozen words out, your [cocks] jutting proud and erect, leaking clear streams of pre-cum down ");
