...
public class SharkGirlScene extends AbstractBoatContent {
    //[Explore Lake]
    public function sharkGirlEncounter():void {
    ...
    public function sharkWinChoices():void {
        ...
        if (player.lust >= 33 && player.gender > 0) {
            ...
            addButtonIfTrue(5, "Dick (Naga)", sharkgirlDickNaga,
                "Req. to be a naga and have a dick with area fitting " + monster.analCapacity(),
                player.isNaga() && player.cockThatFits(monster.analCapacity()) >= 0);
            addButtonIfTrue(1, "Pussy w/69", sharkgirlSixtyNine, "Req. to have non-Naga lower body and a vagina.",
    ...
    public function oceanSharkWinChoices():void {
        clearOutput();
        outputText("The shark girl unable to fight further as she slowly lets the currents carry her.");
        oceanSexMenu();
    }

    public function oceanTigerSharkWinChoices():void {
        clearOutput();
        outputText("The tiger shark girl unable to fight further as she slowly lets the currents carry her.");
        oceanSexMenu();
    }
    ...
    public function oceanSharkspackWinChoices():void {
        spriteSelect(SpriteDb.s_izma);
        clearOutput();
        outputText("The shark girl frenzy breaks formation before your might, leaving their weakest member behind.");
        oceanSexMenu();
    }
    ...
    private function sharkgirlDickNaga():void {
        player.addStatusValue(StatusEffects.SharkGirl, 1, 1);
        clearOutput();
        spriteSelect(SpriteDb.s_sharkgirl);
        var x:Number = player.cockThatFits(monster.analCapacity()); //should be failsafe
        if (monster.lust >= monster.maxOverLust()) outputText("You slither towards the furiously masturbating shark-girl. She lies on her back, desperately trying to relieve herself of her lust. She eyes you for a second, but her focus quickly returns to your own sex, moaning and sighing loudly. You admire the scene for a moment, but decide that she must be punished for her attempt to rape you.\n\n");
    ...
    private function sharkgirlLesboNaga():void {
        clearOutput();
    ...
    internal function sharkLossRape():void {
        ...
        sceneHunter.selectLossMenu([
            ...
            "You're gonna make the girl happy. Which part will you use for such an important task?\n\n"
        );
    ...
    private function sharkGirlGetsDildoed():void {
        ...
        if (player.weaponName != "fists") outputText("put away your [weapon] and ");
