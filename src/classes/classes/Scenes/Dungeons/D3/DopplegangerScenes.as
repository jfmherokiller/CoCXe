...
/**
	...
	public class DopplegangerScenes extends BaseContent
	{
		...
		public function getDemGlasses():void
		{
			...
			if (player.isRace(Races.HUMAN, 1, false))  outputText("  It is honestly surprising, given everything you have encountered since you stepped through that portal what seems years ago, that you are still recognizably human. You wonder if your parents would be able to recognize the battle-hardened " + player.mf("man", "woman") +" clad in " + player.mf("his", "her") +" [armor] standing in front of you to be the wide-eyed youth they bade their heartfelt goodbyes to all that time ago, though.");
        ...
        //I don't want to FIGHT doppel until I READ about doppel.
        public function beforeFight():void {
            clearOutput();
		...
		public function fuckYourself():void
		{
			...
			function dickF():void {
				if (!player.hasVagina()) {
					...
					else if (!player.isFurCovered() && player.tailType != Tail.NONE) outputText("  You clamber on top of [him], the rough mingling of your fur sparking deep urges within you, your animal half dreaming of frenzied, bestial mating in the wild. Your hardening dick presses against [his] [tail] and [he] groans raggedly in response.");
					...
					if (player.hasPerk(PerkLib.Buttslut) || player.hasPerk(PerkLib.MaraesGiftButtslut)) outputText("  You glory in your own decision to transform into an eager butt slut; the doppelganger’s back passage is tight, warm butter, accepting and clenching around your bulging prick like the perfect little boypussy you know it to be.");
					...
					outputText("\n\nYou bury as much of yourself into [him] as you can, driving your bulging length over [his] prostate as you tense, open wide and surge line after line of cum into [his] tunnel. Your mirror image howls, finding a note you never knew your vocal cords could reach as [he] thrashes back into you, [his] [hair] flapping into your face as [he] orgasms in tandem.");
				...
				else if (player.hasCock() && player.hasVagina())
				{
					...
					if (player.isFurCovered()) outputText("  You clamber on top of [him], the rough mingling of your fur sparking deep urges within you, your animal half dreaming of frenzied, bestial mating in the wild. Your dick presses into [his] digitigrade thigh and [he] groans raggedly in response.");
					else outputText(" You clamber on top of [him], enjoying the give and take of [his] "+ (player.isGoo() ? "gooey" : "smooth") + " flesh as you lock your [hips] around their mirror counterparts, making [him] groan raggedly in response.");
			...
			function sharedEnd():void {
				if (!recalling) {
					player.sexReward("vaginalFluids", player.hasCock() ? "Dick" : "Vaginal");
