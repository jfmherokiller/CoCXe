...
public function desertEncounters():void {
	if (flags[kFLAGS.SAMIRAH_AFFECTION] < 20) {
		...
		outputText("You share with her some stories of your latest adventures the naga listening attentively. Once you're done Samirah simply ask you out.\n\n");
		...
		if (flags[kFLAGS.SAMIRAH_FOLLOWER] == 9) {
			...
			outputText("As you explore the desert you run into Samirah atop a nearby dune slowly trailing off further into the desert. She goes along her way, her body weaving back and forth. As you get closer she spots you from far away and slithers up to you. A smiling at the chance to talk to a friendly face.\n\n");
			outputText("\"<i>Good day [name] it's nice of you to visit me all the way in this lost wasteland.</i>\"\n\n");
			outputText("Samirah also ask about the whole deal with the alchemist but for now you have yet to bring something back with you. You share with her some stories of your latest adventures the naga listening attentively. Once you're done, Samirah simply asks you out.\n\n");
			outputText("\"<i>It's been a long and boring day out there and I’m somewhat antsy, are you in the mood for some bonding?</i>\"\n\n");
			menu();
			addButton(1, "Yes", samirahYesSex);
			addButton(3, "No", samirahNoSex);
...
public function samirahMainCampMenu():void {
	...
	if (rand(2) == 0 && flags[kFLAGS.CAMP_UPGRADES_HOT_SPRINGS] >= 4) {
		...
		outputText("She walks away in a rush leaving you to the naga care.\n\n");
...
public function samirahMainCampMenu2():void {
	...
	menu();
	addButton(0, "Appearance", samirahAppearance).hint("Examine Samirah's appearance.");
	addButton(1, "Talk", samirahMainTalkMenu);
	addButton(2, "Sex", samirahMainSexMenu).disableIf(player.lust < 33, "Not aroused enough!");
	//addButton(3, "Items", );
...
public function samirahTalkHer():void {
	clearOutput();
	outputText("What can she tell you about herself? Actually, why did she come to the hellhole that is Mareth in the first place?\n\n");
...
public function samirahMainSexMenu():void {
	clearOutput();
	outputText("Samirah smiles and asks out.\n\n");
...
public function sexMenu():void {
	...
	addButton(1, "Fuck Her", samirahFuckHer).disableIf(!player.hasCock(), "Req. a cock!");
	addButton(6, "FuckHerWild", samirahFuckHerWild).disableIf(!player.hasCock(), "Req. a cock!");
	addButton(2, "Tongue Job", samirahTongueJob).disableIf(!player.hasCock(), "Req. a cock!");
	addButton(7, "Tail Pen", samirahTailPenetration).disableIf(!player.hasVagina(), "Req. a vagina!");
	addButton(12, "Twin Tail Fuck", samirahTwinTailFuck)
		.disableIf(!player.isNaga() && player.tailType != Tail.DRACONIC && player.tailType != Tail.LIZARD && player.tailType != Tail.SALAMANDER && player.tailType != Tail.CAVE_WYRM, "Requires a long (naga, dragon, lizard, salamander, cave wyrm) tail!")
...
public function nagaDefaultSexScene():void {
	...
	function nofitF():void {
		...
		outputText("The kiss continues as both of your bodies rub together sensually, your tails continuing to wrap around one another. There is a pulsing in your [cock " + x1 + "] as it starts to grow hard against " + name + " soft belly. Feeling this, " + name + " gently pulls away, slowly letting your tongue out of her mouth as she does so. Her hands make their way over your shoulders, down your abs, and stop at your [cocks].\n\n");
		outputText("\"<i>You're quite the big boy, aren't you?</i>\" she says as she wraps both of her hands around your now throbbing [cock " + x1 + "]. She starts to slide her hands up and down your length. Sticking out her tongue, she wraps it around the tip and licks at the pre that is starting to leak out.\n\n");
		outputText("A hiss of pleasure escapes your lips as " + name + " strokes and licks at your [cock " + x1 + "], her talented fingers and tongue bringing you into a further state of arousal. " + capitalizeFirstLetter(name) + " stops her caressing and brings your [cock " + x1 + "] to her chest, pressing her breasts around it. She slowly starts to slide her body up and down your shaft, using her tongue to bring some of your pre and lubricate her body. Once she is satisfied with her body's slickness, she quickens her pace.\n\n");
		outputText("You moan in pleasure as " + name + " takes the tip of your member into her mouth and starts to suck. You can feel her deft tongue licking every inch of your cock head. At your limit, you let out a yell as you cum into her mouth. ");
		...
		outputText("You collapse onto the sand and lay there, basking in the warm glow of your orgasm. " + name + " slides onto your chest and gives you a kiss. \"<i>I rather enjoyed that,</i>\" she hisses into your ear, \"<i>We should do this more often.</i>\"\n\n");
	...
	function fitF():void {
		var x1:int = player.cockThatFits(35) + 1;
		...
		outputText("to grow hard against " + name + " soft belly. Feeling this, " + name + " gently pulls away, slowly letting your tongue out of her mouth as she does so. Her hands make their way over your shoulders, down your abs, and stop at your [cocks].\n\n");
		if (player.cockTotal() == 1) outputText("Gently she starts to stroke the length of your [cock " + x1 + "] with one hand while circling the tip with the other. ");
		...
		outputText("A hiss of pleasure escapes your lips as " + name + " strokes your [cocks], her talented fingers bringing you further into a state of arousal. She stops her caress and brings her hand to a scaly covering at her crotch, spreading it wide to reveal her soft pussy.\n\n");
	...
	function vagF():void {
		...
		outputText("Traveling down, you pause at her collarbone, letting go of her hips to bring your hands up to her perfectly rounded breasts. A moan escapes " + name + "'s lips as you massage her erect nipples. Your mouth continues its trek down " + name + "'s supple body and you make sure to pause on each breast, circling inward and stopping on each nipple to suck gently on them.\n\n");
		outputText("Once more your hands move down " + name + "'s body, making their way across her sides as your mouth simultaneously kisses at the smooth flesh of her exposed belly. Goose pimples slowly begin to appear, denoting how much she enjoys it. You pause at her hips, hovering over the slit at her crotch. One of your hands slide down to the slit and you start to stroke at it gently as you kiss at the area around it. Your partner shudders a bit, overcome by the sensations. Slowly you slide a finger into her, hearing a gasp of pleasure as you move it around inside her. ");
		...
		outputText("You twist your fingers deeper inside of her, feeling the moist walls of her love canal press around your finger. Carefully you move your hand and push another of your fingers inside, then a third, moving them to massage her innermost depths. " + capitalizeFirstLetter(name) + "'s whole body is shuddering with arousal at your touch. It would be easy enough to bring her to a climax like this, but you decide that you want to make a slight change to the situation.\n\n");
		...
		outputText("You untangle the end of your tail from hers and bring it up between the two of you, sliding it between your [breasts] and inserting it sensually into your mouth. You suck on the end, making sure to coat it liberally before bringing it down to " + name + "'s awaiting slit.\n\n");
	...
	function glessF():void {
		if (player.gender == 0) {
			outputText(capitalizeFirstLetter(name) + " looks down and suddenly giggles at your stark lack of fun-bits. You turn away in embarrassment and start to head back to your camp.\n\n");
			outputText("\"<i>Wait,</i>\" " + name + " reaches out and places a hand on your shoulder. \"<i>I didn't mean to insult you. I've never seen anyone else quite like you.</i>\" She slides her hand down your body. \"<i>I wonder if you can show me just what a body like yours can do.</i>\"\n\n");
			outputText("Enjoying this new shift in the conversation, you");
		} else outputText("You")
		outputText(" coil around her and position your head over the scaly covering over her pussy. A soft hiss escapes " + name + " as you slide your tongue into her, twirling it around to caress every inch of her. Her hands make their way to your head and press you deeper into her.\n\n");
